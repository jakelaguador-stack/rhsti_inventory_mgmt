...
/*--------------------------------------------------------------*/
 ...
 function updateLastLogIn($user_id)
  ...
  function find_by_groupName($val)
  {
    ...
    $result = $db->query($sql);
    return ($result && $db->num_rows($result) === 0 ? true : false);
  }
  /*--------------------------------------------------------------*/
  /* Find group level
  /*--------------------------------------------------------------*/
  ...
  function columnExists($table, $column) {
    ...
    $t = $db->escape($table);
    $c = $db->escape($column);
    $result = $db->query("SHOW COLUMNS FROM {$t} LIKE '{$c}'");
  ...
  /*--------------------------------------------------------------*/
   ...
   /*--------------------------------------------------------------*/
    function join_product_table(){
      ...
      $has_remarks = function_exists('columnExists') && columnExists('products', 'remarks');
      ...
      $sql .= $has_remarks ? ", p.remarks" : ", '' AS remarks";
      $sql .= ", p.categorie_id, c.name AS categorie, m.file_name AS image";
      $sql .= " FROM products p";
    ...
    function ensure_product_name_index_is_not_unique(){
      global $db;
      $result = $db->query("SHOW INDEX FROM products WHERE Key_name = 'name'");
      if(!$result || $db->num_rows($result) === 0){
        return;
      }

      $indexes = $db->while_loop($result);
      foreach($indexes as $index){
        if(isset($index['Key_name']) && $index['Key_name'] === 'name' && isset($index['Non_unique']) && (int)$index['Non_unique'] === 0){
          $db->query("ALTER TABLE products DROP INDEX name");
          break;
        }
      }
    }
  ...
  function ensure_product_unit_column(){
      global $db;
      $result = $db->query("SHOW COLUMNS FROM products LIKE 'unit'");
      if($result && $db->num_rows($result) === 0){
        $db->query("ALTER TABLE products ADD COLUMN unit VARCHAR(100) DEFAULT '' AFTER quantity");
      }
    }

  function ensure_product_serial_receipt_columns(){
      global $db;
      $serial_result = $db->query("SHOW COLUMNS FROM products LIKE 'serial_number'");
      if($serial_result && $db->num_rows($serial_result) === 0){
        $db->query("ALTER TABLE products ADD COLUMN serial_number VARCHAR(100) DEFAULT '' AFTER media_id");
      }

      $receipt_result = $db->query("SHOW COLUMNS FROM products LIKE 'receipt_number'");
      if($receipt_result && $db->num_rows($receipt_result) === 0){
        $db->query("ALTER TABLE products ADD COLUMN receipt_number VARCHAR(100) DEFAULT '' AFTER serial_number");
      }
    }

  function ensure_product_remarks_column(){
      global $db;
      $result = $db->query("SHOW COLUMNS FROM products LIKE 'remarks'");
      if($result && $db->num_rows($result) === 0){
        $db->query("ALTER TABLE products ADD COLUMN remarks TEXT DEFAULT '' AFTER quantity");
      }
    }
  /*--------------------------------------------------------------*/
  /* Function for Finding all product name
  ...
  function find_product_group_totals($name, $unit, $categorie_id){
    global $db;
    $name = remove_junk($db->escape($name));
    $unit = trim(remove_junk($db->escape($unit)));
    if($unit === '') { $unit = 'piece'; }
    $categorie_id = (int) $categorie_id;
    $sql = "SELECT SUM(quantity) AS quantity, SUM(COALESCE(used,0)) AS used ";
    $sql .= "FROM products WHERE name = '{$name}' AND COALESCE(NULLIF(unit,''),'piece') = '{$unit}' AND ";
    $sql .= "categorie_id = '{$categorie_id}'";
    $result = $db->query($sql);
    if(!$result) {
      return ['quantity' => 0, 'used' => 0, 'available' => 0];
    }
    $data = $db->fetch_assoc($result);
    $quantity = (int) ($data['quantity'] ?? 0);
    $used = (int) ($data['used'] ?? 0);
    return ['quantity' => $quantity, 'used' => $used, 'available' => max(0, $quantity - $used)];
  }

  function update_group_product_used($qty, $name, $unit, $categorie_id){
    global $db;
    $remaining = (int) $qty;
    $name = remove_junk($db->escape($name));
    $unit = trim(remove_junk($db->escape($unit)));
    ...
    $sql = "SELECT id, quantity, COALESCE(used,0) AS used ";
    $sql .= "FROM products WHERE name = '{$name}' AND COALESCE(NULLIF(unit,''),'piece') = '{$unit}' AND ";
    $sql .= "categorie_id = '{$categorie_id}' ORDER BY id ASC";
    $result = $db->query($sql);
    if(!$result || $db->num_rows($result) === 0) {
      return false;
    }
    ...
    while($row = $db->fetch_assoc($result)) {
      ...
      $avail = max(0, (int)$row['quantity'] - (int)$row['used']);
      if($avail <= 0) {
        continue;
      }
      $take = min($avail, $remaining);
      $update = "UPDATE products SET used = GREATEST(0, COALESCE(used,0) + {$take}) WHERE id = " . (int)$row['id'];
      $db->query($update);
